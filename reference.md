...
**specification:** `*Lattice.GoogleProtobufAny` — The path for the Protobuf task definition, and the complete task data.
...
**updateStartTime:** `*Lattice.Timestamp` — If provided, returns tasks which have been updated since the given time.
...
**assignee:** `*Lattice.Principal` — A filter for tasks assigned to a specific principal.
...
**statusFilter:** `*Lattice.TaskStreamRequestStatusFilter` — A filter for task statuses (inclusive or exclusive).
...
<details><summary><code>client.Tasks.StreamManualControlFrames(TaskID, request) -> Lattice.StreamManualControlFramesResponse</code></summary>
<dl>
<dd>
...
<details><summary><code>client.Objects.ListDeletedObjects() -> *Lattice.ListDeletedObjectsResponse</code></summary>
<dl>
<dd>
...
**pageToken:** `*string` — Opaque cursor returned by a prior response to continue paging.
...
**maxPageSize:** `*int` — Maximum number of records to return in a single response. Server enforces an upper bound.
